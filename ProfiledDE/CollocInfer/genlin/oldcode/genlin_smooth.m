function [coefs,fd_cell] = genlin_smooth(Ycell,Tcell,wts,basis_cell, ...
    lambda,pars,alg,more)
%genlin_smooth: produces an exactly calculated smooth with penalty 
%           corresponding to a forced, linear differential equation. 
%
% INPUTS
%
...
pmat = more.mat;
pmat(sub2ind([n n],more.sub(:,1),more.sub(:,2))) = ...
        pars(1:size(more.sub,1));
...
if(~isempty(more.force))
    ...
    b(sub2ind(size(b),more.force_sub(:,1),more.force_sub(:,2))) = ...
        pars((size(more.sub,1)+1) :  ...
             (size(more.sub,1)+size(more.force_sub,1)));
...
coefs = (Zmat'*diag(wts)*Zmat + ...
    (Dphimat-phimat)'*diag(qvals)*(Dphimat-phimat))\...
