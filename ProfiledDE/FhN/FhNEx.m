...
%
% We need to define functions that will calculate this function and
% its derivatives.
...
% First we need a function to calculate the function directly. 
...
startpars = pars + jitter*randn(npar,1);
...
Ycell = path_cell;                
for i = 1:nvar
    Ycell{i} = path_cell{i} + sigma*randn(N);
end
...
% the weight is constant across observations in each component. 
...
% then take the mean across all values. 
