...
% Users Manuel for more detailed commentary. What commentary there is in
...
% experiments:
%
% * the use of the input |parind| to allow some parameters to vary across
%   replications
...
fn.dfdp     = @fhndfdp;      % Derivative wrt parameters
...
% repeated time series is that some parameters may be common across time
...
%
% The main additional input into code is an matrix of indices called 
...
% across the three replications with its three values stored in positions
...
full_time = zeros(length(tspan),3);
full_path = zeros(length(tspan),2,3);

plot_time = zeros(length(tfine),3);
plot_path = zeros(length(tfine),2,3);
...
% Other observed optimization control
...
% Optimization control within profiling
